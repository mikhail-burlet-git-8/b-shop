...
class Flash {
    ...
    public function get(): ?FlashMessage {
        ...
        $message = $this->session->get( self::MESSAGE_CLASS_KEY );
    ...
    private function flash( string $message, string $name ): void {
        $this->session->flush( self::MESSAGE_KEY, $message );
        $this->session->flush( self::MESSAGE_CLASS_KEY, config( "flash.$name", '' ) );
